  ...
  def post_install
    ...
    work_dir.cd do
      ...
      (work_dir/"takari-maven-plugin").cd do
        ...
        system "echo", "\"Run [#{libexec}/bin/mvn -e -X -Dmaven.repo.local=#{work_dir}/repository install]\""
        system "#{libexec}/bin/mvn", "-e", "-X", "-Dmaven.repo.local=#{work_dir}/repository", "install"
      end
    end
